...
!function($, B) { 
   ...
   * $.api(path, type, data, callback)
   * $.api({ ... })
   */
  $.api = function(path, type, data, callback) {
    var onSuccess, onError;
      
    // if function is fourth arg, no shifting
    if ($.isFunction(callback)) {
      onSuccess = callback;
    // if function is third arg, shift forward
    ...
    // if function is second arg, shift forward
    } else if ($.isFunction(type)) {
      onSuccess = type;
      data = {};
      type = 'GET';
    // finally, consider parsing path as config object
    } else if ($.isPlainObject(path)) {
      onSuccess = path.success;
      onError = ( $.isFunction(path.error) && path.error ) || defaultOnError;
      data = path.data || {};
      type = path.type || 'GET'; 
      path = path.path;
    } 
    ...
    if (temp !== 'PUT' && temp !== 'POST' && temp !== 'DELETE' && temp !== 'GET') {
      data = type;
      type = 'GET';
    }

    var method = type = type.toUpperCase();
    ...
    if (!$.isFunction(onError)) {
      onError = function(error) {
        console.error('$.api(' + type + ':' + path + '): ' + error);
      };
    }
    ...
    var params = {
      ...
      'error': function(jqXHR, textStatus, errorThrown) {
        ...
        if (error) {
          ...
          console.error('$.api(' + type + ':' + path + '): ' + textStatus, errorThrown, jqXHR);
        }
      }
    };
  ...
  $.views.helpers({
    ...
    /**
     * Alias for Sugar.js' String.stripTags(tag1, tag2, ...)
     */
    stripTags: function( val /*, tag1, tag2, ... */ ) {
      // TODO: expand such that tags can be used to specify stripping behavior
      return ( val || '' ).stripTags();
    },

    /**
     * Alias for Sugar.js' String.trim()
     */
    trim: function( val ) {
      return ( val || '' ).trim();
    },

    stripTagsAndTrim: function( val /*, tag1, tag2, ... */ ) {
      // TODO: expand such that tags can be used to specify stripping behavior
      return ( val || '' ).stripTags().trim();
    }
